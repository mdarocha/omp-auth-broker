import {
    ...
    type AuthStorage,
    type OAuthProvider,
    type SqliteAuthCredentialStore,
} from "@oh-my-pi/pi-ai";
...
export async function controlApi(request: Request, context: ControlContext): Promise<Response> {
    try {
        ...
        if (pathname === "/api/snapshot") {
            ...
            return forwardUpstream(await fetch(`${context.brokerBase}/v1/snapshot`));
        }
...
async function usage(context: ControlContext): Promise<Response> {
    ...
    const [usagePayload, clientsPayload] = (await Promise.all([usageResponse.json(), clientsResponse.json()])) as [
...
async function login(request: Request, context: ControlContext): Promise<Response> {
    ...
    let resolveStart: (value: { sessionId: string; url: string; instructions?: string; needsCode: boolean }) => void;
...
async function loginCode(encodedSessionId: string, request: Request, context: ControlContext): Promise<Response> {
    const sessionId = decodeURIComponent(encodedSessionId);
    const session = context.sessions.get(sessionId);
...
async function logout(request: Request, context: ControlContext): Promise<Response> {
    const provider = await readProvider(request);
    if (provider instanceof Response) {
        return provider;
    }
