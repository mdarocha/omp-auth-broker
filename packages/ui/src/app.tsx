...
function App() {
	...
	useEffect(() => {
		...
		const poll = async () => {
			try {
				...
				if (!cancelled) setLogin(current => (current ? { ...current, state: "error", message: errorMessage(error) } : current));
			}
		};
	...
	async function startLogin(provider: Provider) {
		...
		setCodeError(null);
		try {
			const started = await api<{ sessionId: string; url: string; instructions?: string; needsCode: boolean }>("/api/login", {
				method: "POST",
			...
			setLogin({ sessionId: "", provider, url: "", needsCode: false, state: "error", message: errorMessage(error) });
		} finally {
			setLoginBusy(null);
		}
	}
	...
	const reportRows = useMemo(
		() => usage.phase === "ready" ? usage.data.reports.flatMap(report => report.limits.map(limit => ({ report, limit }))) : [],
		[usage],
	);
			...
			<main>
				<section class="section" aria-labelledby="accounts-heading">
					<div class="section__head">
						...
						<button class="button button--primary" type="button" aria-expanded=${pickerOpen} aria-controls="provider-picker" onClick=${() => setPickerOpen(value => !value)}>
					...
					<${Accounts} snapshot=${snapshot} removing=${removing} removeError=${removeError} onRetry=${loadSnapshot} onChooseProvider=${() => setPickerOpen(true)} onRemoveProvider=${removeProvider} />
				</section>
