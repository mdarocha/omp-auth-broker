...
export function Accounts({ snapshot, removing, removeError, onRetry, onChooseProvider, onRemoveProvider }: AccountsProps) {
	return html`
		...
		<div class="table-frame">
			...
			${snapshot.phase === "error" && html`
				<div class="state-block">
					<p class="notice notice--error" role="alert">Accounts could not be loaded: ${snapshot.message}</p>
					<button class="button button--secondary" type="button" onClick=${onRetry}>Try again</button>
				</div>
			`}
			${snapshot.phase === "ready" && snapshot.data.credentials.length === 0 && html`
				<div class="empty-state">
					...
					<button class="button button--secondary" type="button" onClick=${onChooseProvider}>Choose provider</button>
				</div>
			`}
			${snapshot.phase === "ready" && snapshot.data.credentials.length > 0 && html`
				<table>
					<thead><tr><th>Provider</th><th>Identity</th><th>Type</th><th>Status</th><th>Expires</th><th>Refresh in</th><th><span class="sr-only">Actions</span></th></tr></thead>
					<tbody>
						${snapshot.data.credentials.map(entry => {
							...
							return html`<tr key=${entry.id}>
								...
								<td><span class=${`status status--${disabled ? "disabled" : "active"}`}>${disabled ? "Disabled" : "Active"}</span></td>
								<td class="numeric"><span>${formatDate(entry.credential.expires)}</span>${entry.credential.expires && html`<small>${relativeTime(entry.credential.expires - Date.now())}</small>`}</td>
								<td class="numeric">${entry.rotatesInMs === null ? "—" : relativeTime(entry.rotatesInMs)}</td>
								<td class="action-cell"><button class="button button--danger" type="button" disabled=${removing !== null} onClick=${() => onRemoveProvider(entry.provider)}>${removing === entry.provider ? "Removing…" : "Remove"}</button></td>
							</tr>`;
						})}
					</tbody>
				</table>
