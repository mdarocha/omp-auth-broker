	...
	return html`
		<aside class=${`login-flow login-flow--${login.state}`} aria-labelledby="login-heading">
			<div class="login-flow__head">
				...
				<button class="icon-button" type="button" aria-label="Dismiss authorization panel" onClick=${onDismiss}>×</button>
			</div>
			${login.state === "pending" && html`
					...
					<form class="code-form" onSubmit=${onSubmitCode}>
						...
						<div class="code-form__controls">
							<input
								...
								required
							/>
							<button class="button button--secondary" type="submit" disabled=${codeBusy || !code.trim()}>${codeBusy ? "Sending…" : "Send code"}</button>
						</div>
