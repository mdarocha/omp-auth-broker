...
export function ProviderPicker({ providers, loginBusy, onRetry, onSelect }: ProviderPickerProps) {
	return html`
		<div class="provider-picker" id="provider-picker">
			...
			${providers.phase === "ready" && providers.data.length > 0 && html`
				<ul class="provider-list">
					${providers.data.map(provider => html`
						<li key=${provider.id}>
							<button type="button" onClick=${() => onSelect(provider)} disabled=${loginBusy !== null}>
								<span>${provider.name}</span>
								<span class="provider-list__meta">${loginBusy === provider.id ? "Starting…" : provider.pasteCode ? "Paste code" : provider.id}</span>
							</button>
						</li>
					`)}
				</ul>
