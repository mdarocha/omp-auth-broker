        ...
        <section class="section" aria-labelledby="usage-heading">
            ...
            <div class="usage-block">
                <div class="subhead">
                    ...
                    <span>${reportRows.length} ${reportRows.length === 1 ? "window" : "windows"}</span>
                </div>
                        ...
                        html`
                            <div class="state-block">
                                <p class="notice notice--error" role="alert">
                                    Usage could not be loaded: ${usage.message}
                                </p>
                    ...
                    ${
                        ...
                        html`
                            <table>
                                ...
                                <tbody>
                                    ${reportRows.map(({ report, limit }) => {
                                        const fraction = usedFraction(limit.amount);
                                        return html`<tr key=${`${report.provider}-${report.fetchedAt}-${limit.id}`}>
                                            <td class="provider-cell">${report.provider}</td>
                                            <td>${reportIdentity(report)}</td>
                                            <td>
                                                ${limit.label}${limit.window?.label && html`<small>${limit.window.label}</small>`}
                                            </td>
                                            <td class="numeric usage-amount">
                                                <span
                                                    >${formatAmount(limit.amount.used, limit.amount.unit)} /
                                                    ${formatAmount(limit.amount.limit, limit.amount.unit)}</span
                                                >${fraction !== undefined && html`<progress max="1" value=${Math.min(Math.max(fraction, 0), 1)} aria-label=${`${Math.round(fraction * 100)} percent used`}></progress>`}
                                            </td>
                                            <td class="numeric">
                                                ${limit.window?.resetsAt ? html`<span>${formatDate(limit.window.resetsAt)}</span><small>${relativeTime(limit.window.resetsAt - Date.now())}</small>` : "—"}
                                            </td>
                                            <td>
                                                <span class=${`status status--${limit.status || "unknown"}`}
                                                    >${limit.status || "Unknown"}</span
                                                >
                                            </td>
                                        </tr>`;
                                    })}
                                    ...
                                    ${usage.data.clients.map((client) => {
                                        ...
                                        return html`<tr key=${client.installId}>
                                            <td>
                                                ...
                                                ><small class="mono">${client.installId}</small>
                                            </td>
                                            <td>${client.providers.map((item) => item.provider).join(", ") || "—"}</td>
                                            <td class="numeric">${number.format(totals.requests)}</td>
                                            <td class="numeric">${number.format(totals.input)}</td>
                                            <td class="numeric">${number.format(totals.output)}</td>
                                            <td class="numeric">${number.format(totals.cache)}</td>
                                            <td class="numeric numeric--strong">
